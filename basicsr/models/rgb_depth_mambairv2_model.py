...
class RGBDepthMambaIRv2Model(MambaIRv2Model):
    """Training and tiled-evaluation wrapper for aligned RGB-depth SR."""
    ...
    def test(self):
        ...
        test_net.eval()
        with torch.no_grad():
            outputs = [test_net(rgb[..., top, left], depth[..., top, left]) for top, left in slices]
            merged = torch.zeros(
