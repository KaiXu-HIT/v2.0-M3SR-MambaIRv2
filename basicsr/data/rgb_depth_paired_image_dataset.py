...
class RGBDepthPairedImageDataset(data.Dataset):
    """Aligned LR RGB, LR depth, and HR RGB dataset for Stage-1 B1.

    Depth is converted to one scalar channel and robustly normalized on the
    complete LR image before cropping. RGB, depth, and GT then share exactly
    the same crop and spatial augmentation.
    """
    ...
    def __init__(self, opt):
        ...
        if self.io_backend_opt.get('type') != 'disk':
            raise NotImplementedError('RGBDepthPairedImageDataset currently supports disk folders only.')
        ...
        depth_roots = _root_list(opt['dataroot_depth'], 'dataroot_depth')
        if not (len(gt_roots) == len(lq_roots) == len(depth_roots)):
            raise ValueError('dataroot_gt, dataroot_lq, and dataroot_depth must have equal lengths.')

        self.paths = []
        for gt_root, lq_root, depth_root in zip(gt_roots, lq_roots, depth_roots):
    ...
    def _normalize_depth(self, depth):
        ...
        if not finite.all():
            depth = depth.copy()
            depth[~finite] = 0.0
        low, high = np.quantile(depth, [self.depth_percentile_low, self.depth_percentile_high])
    ...
    def __getitem__(self, index):
        ...
        if img_lq.shape[:2] != img_depth.shape[:2]:
            raise ValueError(
                'B1 requires aligned LR RGB and depth images, but got '
